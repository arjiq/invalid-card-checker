...
const validateCred = array => {
    ...
    for(let x = 0; x < newArray.length; x++) {
        if ( x === 0) {
            if (newArray[0] * 2 > 9) {
                newArrayDoubled.push(newArray[0]* 2 - 9)
            } 
            else {
                newArrayDoubled.push(newArray[0]* 2)
            }
        }
    else if( x % 2 === 0 ) {
        if (newArray[x] * 2 > 9) {
            newArrayDoubled.push(newArray[x]* 2 - 9)
        } 
        else {
            newArrayDoubled.push(newArray[x]* 2)
        }
    }
    else {
        newArrayDoubled.push(newArray[x])
    }
    
    }
    let sum = 0;
    for (let i = 0; i < newArrayDoubled.length; i++) {
        sum += newArrayDoubled[i]
    }
...
const idInvalidCompanies = array => {
    ...
    noDupes = arrayOfCompanies.filter(function(item, pos) {
        return arrayOfCompanies.indexOf(item) == pos
    })
    console.log(noDupes)
}
